...
public class RequestViewFunction {
	...
	public static void setDataIn_Choice_Obekt_na_Izpitvane_Request(Choice choice_obekt_na_izpitvane_request,
			Request tamplateRequest) {
	...
	public static void addNewObjectIn_Choice_Obekt_na_Izpitvane_Request(JFrame parent,
	...
	public static void setTextIn_Text_Area_List_Izpitvan_Pokazatel(JTextArea txtArea_list_izpitvan_pokazatel,
			Request tamplateRequest) {
		if (tamplateRequest != null) {
			List<String> list_String_I_P_Tamplate = new ArrayList<String>();
			for (IzpitvanPokazatel izpitPokazatelFormTamplate : get_List_Izpitvan_pokazatel_From_Request(
					tamplateRequest)) {
	...
	public static String generateTxtInDescriptGrupSampleNew(Choice choice_Period, String sampleCount,
...
//		 String str_period_sample = DateChoice.get_str_period_sample();
		...
		if (original_Description_sample_group.length() > 0)
			txtInDescriptGrupSample = original_Description_sample_group;
	...
	public static String GenerateStringRefDateTimeFromRequest(Request request) {
		...
		if (compaRefDateTime(masiveReffDateTime)) {
			...
			for (int i = 0; i < masiveReffDateTime.length; i++) {
				...
				date_time_reference = date_time_reference + request.getRecuest_code() + "-" + masiveSampleCode[i]
						+ " / " + masiveReffDateTime[i] + "   ";
			}
	...
	public static Boolean incorrectReception_Date_Period(JTextField txtFld_date_reception) {
		...
		if (reception_date_period.length() > 10) {
			...
			if (reception_date_period.indexOf("÷") > 0) {
				...
				endData = reception_date_period
						.substring(reception_date_period.indexOf("÷") + 1, reception_date_period.length()).trim();
