...
//		choiceDobiv.addItem("");
	...
	static String generate_strStoinostiDobiv_Nuclide(Choice choiceDobiv) {
		...
		if (!choiceDobiv.getSelectedItem().trim().isEmpty()) {
			...
			if(listDobivi.size()>0 && !listDobivi.get(0).getMetody().getCode().contains("М.ЛИ-РХ-03")){
				strrazmernostDobiv ="ч.е. , ";
			}
			for (Dobiv dobiv : listDobivi) {
				valueDobiv = dobiv.getValue_result();
				if(!listDobivi.get(0).getMetody().getCode().contains("М.ЛИ-РХ-03") && valueDobiv > 1 ){
			...
			if(strStoinostiDobiv_Nuclide.trim().isEmpty()){
				 ...
				 if(!dobiv.getMetody().getCode().contains("М.ЛИ-РХ-03")){
						strrazmernostDobiv ="ч.е. , ";
					}
				 valueDobiv = dobiv.getValue_result();
					if(!dobiv.getMetody().getCode().contains("М.ЛИ-РХ-03") && valueDobiv > 1 ){
						valueDobiv = valueDobiv/100;
					}
				 strStoinostiDobiv_Nuclide += dobiv.getNuclide().getSymbol_nuclide() + " " + valueDobiv
					+ strrazmernostDobiv;
